...
def extract_singlescale(net, img,top_k=10000 ,image_name=None):
    ...
    return predictions
def extract_singlescale_desc(net, img,top_k=10000 ,image_name=None):
    old_bm = torch.backends.cudnn.benchmark
    torch.backends.cudnn.benchmark = False
    shape = img.shape
    X, Y, S, C, Q, D = [], [], [], [], [], []
    ...
    with get_model(config['model']['name'])(**config['model']) as net:
        for i, data in tqdm(enumerate(dataset)):
            ...
            if args.single==True:
                ...
                predictions = extract_singlescale_desc(net, data['image'],top_k=args.top_k,image_name=folder_name+'_'+image_name)
            else:
