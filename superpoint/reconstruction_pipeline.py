...
def import_features(images, paths, args):
    ...
    for image_name, image_id in tqdm(images.items(), total=len(images.items())):
        features_path = os.path.join(paths.image_path, '%s.%s.npz' % (image_name, args.method_name))
...
def match_features(images, paths, args):
    ...
    for raw_pair in tqdm(raw_pairs, total=len(raw_pairs)):
        ...
        features_path1 = os.path.join(paths.image_path, '%s.%s.npz' % (image_name1, args.method_name))
        features_path2 = os.path.join(paths.image_path, '%s.%s.npz' % (image_name2, args.method_name))
